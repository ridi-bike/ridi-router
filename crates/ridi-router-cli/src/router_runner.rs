...
mod tests {
    ...
    use ridi_router_routing::{
        Coords as RoutingCoords, RouteComputation, RouteMode, RouteStatElement, RouteStats,
        RouterRules, RoutingError, RoutingGenerationError, RoutingOpenError,
    };
    ...
    fn test_route_stats(len_m: f64) -> RouteStats {
        RouteStats {
            len_m,
            junction_count: 2,
    ...
    fn test_computation() -> RouteComputation {
        RouteComputation {
            routes: vec![ridi_router_routing::ComputedRoute {
                coords: vec![(2.0, 2.0), (3.0, 3.0)],
                stats: test_route_stats(1_234.0),
            }],
        }
    }
    ...
    #[test]
    fn generate_route_cli_renders_routing_generation_error() {
        let error = super::RouterRunnerError::Routing {
            error: RoutingError::Generate(RoutingGenerationError::PointNotFound {
                point: "start point",
            }),
        };
    ...
    #[test]
    fn router_runner_uses_routing_result_type() {
        let computation = test_computation();
