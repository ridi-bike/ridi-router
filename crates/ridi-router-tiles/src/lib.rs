...
use rmdf::generator::{MultiPbfGenerator, TileGenerator};
...
mod tests {
    ...
    #[test]
    fn tile_generation_error_reports_missing_input() {
        let missing_input = unique_path("missing-input.osm.pbf");
        let output_dir = unique_path("output-dir");

        let error = generate_tiles(TileGenerationRequest {
            input: TileInputSource::File(missing_input.clone()),
    ...
    #[test]
    fn tile_generation_public_api_does_not_expose_clap_or_cli_types() {
        let cargo_toml =
            std::fs::read_to_string(concat!(env!("CARGO_MANIFEST_DIR"), "/Cargo.toml")).unwrap();
        assert!(!cargo_toml.contains("clap.workspace"));
        assert!(!cargo_toml.contains("gpx.workspace"));
        assert!(!cargo_toml.contains("tiny_http"));
    }
}
