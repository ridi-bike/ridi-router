...
run-lv-round:
	cargo run --release -- dual --debug-file ./map-data/debug.db --input /map-data/latvia-latest.osm.pbf --output map-data/lv.gpx --cache-dir ./map-data/cache/latvia --rule-file map-data/rules-prefer-unpaved.json round-trip --center {{gps-test-from-lat}},{{gps-test-from-lon}} --bearing 0 --distance 100000
