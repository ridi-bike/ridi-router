...
run-load-pbf-show:
	cargo run --features=debug-split-gpx -- generate-route --input map-data/latvia-latest.osm.pbf --cache-dir map-data/cache/lv --output map-data/output.gpx --rule-file rule-examples/rules-empty.json start-finish --start {{gps-test-from-lat}},{{gps-test-from-lon}} --finish {{gps-test-to-lat}},{{gps-test-to-lon}}
	gpxsee map-data/output.gpx &
